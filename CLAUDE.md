...
- [当前版本完整主路径_落地进度](Design/进度/当前版本完整主路径_落地进度.md) — **进行中**：7 轮设计讨论闭环（[当前版本完整主路径_MVP设计](Design/当前版本完整主路径_MVP设计.md)）；待启动引擎扩展（末尾位 tag 池抽取 / 自省结构改造 / 背景 fallback）+ CSV 回看
...
- [intro_全盘重新设计_预启动](Design/进度/intro_全盘重新设计_预启动.md) — **进行中**：美术表现轨精调收尾（IntroSequence 13 层永久接管屏幕渲染）；事件设计轨未开始，前置阻塞为解除 `_render_event_background` 中 girl_enter 硬编码
...
- [UI风格快速翻调_demo期进度](Design/进度/UI风格快速翻调_demo期进度.md) — **进行中**：Phase 1+1.5 已落（字体分工 + 墨印阴刻 + 斑驳层 + 朱色收紧），等数字定性化裁断后 mock 转正式
