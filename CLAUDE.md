...
- [当前版本完整主路径_落地进度](Design/进度/当前版本完整主路径_落地进度.md) — **Phase B 阶段 1 + 2 + 阶段 3 全部 NPC 批次已收口**（2026-05-12，何/秦/周/程 35 事件 + sys_reflection p1 5 行 condition 差分全部真实 + 4 地点 transition 全部落地 + 工程 CSV + 跑测通过；程批次核心兑现"4 outcome 核心台词一字不差·话术学最深暴露" + 温水机制真正运作机制揭示 + 精神镜面层次精细演化设计 + §23/§24 认知陷阱沉淀）。Phase A Step 1-11 已完成（demo_mode 5 开关全生效）。**Phase B 阶段 3 尾声待新会话启动**——拜师 chain 4 屏 + 最终自省（独立批次，按 [[事件卡撰写流程规范]] §七 9 步 SOP 走）。**结构性数字**（4 NPC / 4 地点 / 4 骨架 / chain 4 屏 / 1+3+1 自省）沿用；**资源调度白名单**（spirit/gold + 3 鉴定属性）用户决议；衍生议题保留：事件背景渲染层架构（委托 [[intro_全盘重新设计_预启动]]）
...
- [文字马赛克美术背景_落地进度](Design/进度/文字马赛克美术背景_落地进度.md) — 基础设施稳定；决策点 7 收口搁置 + 决策点 8 边界处理收口；边缘 fade 锋利度精挑作储备。**2026-05-12 hybrid 决议后**：mosaic 实际渲染范围缩小到 pond_*（自省事件），其他事件背景走 plain PNG；全场景 mosaic 化作技术债务追踪见 [[文字马赛克全局化_预启动]]
...
- [阶段2回看调整_由阶段3触发_预启动](Design/进度/阶段2回看调整_由阶段3触发_预启动.md) — 当前快照范围被 [当前版本完整主路径_MVP设计](Design/当前版本完整主路径_MVP设计.md) 吸收；触发型机制保留
- [正式UI迁移_预启动](Design/进度/正式UI迁移_预启动.md) — **预启动**：正式美术风格 UI 替换当前 demo 期临时 UI；触发条件待开篇 + 主玩法循环验证
...
- [事件切换呼吸感_落地进度](Design/进度/事件切换呼吸感_落地进度.md) — **已落地**（2026-05-12）：IntroSequence plain mode + mode 切换 tween 化 + main_game fade 调度器 + 合并 _trigger_reflection_intro_fade + 同图 pulse 串行节奏（modulate 颜色变暗不动 α）+ 3 项实施期 bug/增强 + **Outro 收口流程**（sys_final_reflection → 涟漪 → "待续" → "开始" → 就地重启进 sys_opening_reflection）+ Auto-confirm 中间态吞噬（chain_entry/2/3 + is_ending_event）+ luchengyue 美术入库；MVP §十一 outro 章节同步更新
