...
- [当前版本完整主路径_落地进度](Design/进度/当前版本完整主路径_落地进度.md) — **Phase B 阶段 1 已收口**（2026-05-10，药铺·何线 8 事件全部落地 + 工程 CSV + 跑测通过 + [[事件卡撰写流程规范]] + [[stage_2_GPT-Image-2提示词]]）。Phase A Step 1-11 已完成（demo_mode 5 开关全生效）。**Phase B 阶段 2 待新会话启动**——家+市场·秦素娘批次（含骨架 + 6 填充 + 中段自省 met_qin 分支 + 家 transition + 秦素娘美术资产）。**结构性数字**（4 NPC / 4 地点 / 4 骨架 / chain 4 屏 / 1+3+1 自省）沿用；**资源调度白名单**（spirit/gold + 3 鉴定属性）用户决议；衍生议题保留：事件背景渲染层架构（委托 [[intro_全盘重新设计_预启动]]）/ 需求 2 presentation 差分（4 段差分都写好后统一改造）
