...
- [前端表现卡牌化_MVP草案](Design/zhengdao-cards/进度/前端表现卡牌化_MVP草案.md) — **草案 v3 + 工程启动 checklist**（2026-05-15）：前端交互改卡牌(引擎层保留 + 选项层新增调度);**新仓库 `zhengdao-cards`** + Design 共用决议;核心机制(事件-选项-结果三层卡循环 / 选项调度 / 鉴定+心性资源闭环 / 资源卡+标记 / 结果卡复用 outcome)就位 + **§2.2 内核明确化(普通事件玩法 / chain 叙事承担分离 — Phase B 歧途反向印证)** + MVP 4 步迭代路径;9 条 Design 分区边界 case 全部裁决;§7.8 工程启动 4 阶段 checklist(A Design 整理 / B 仓库初始化 / C 引擎迁移 / D MVP 1 期前置)待执行。**vibe-test 进入「养分源」状态**
