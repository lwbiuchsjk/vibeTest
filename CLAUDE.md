...
- [当前版本完整主路径_落地进度](Design/进度/当前版本完整主路径_落地进度.md) — **Phase B 阶段 1 + 2 已收口**（2026-05-11，何线 + 秦线 17 事件 + sys_reflection p1 5 行 condition 差分 + fallback 通用文本 + 工程 CSV + 跑测通过；需求 2 改造提前实施完成）。Phase A Step 1-11 已完成（demo_mode 5 开关全生效）。**Phase B 阶段 3 待新会话启动**——剩余 NPC 批次（周既明 / 程叔衡，按 [[事件卡撰写流程规范]] §七 9 步 SOP 走）+ 剩余收尾（met_zhou / met_cheng 真实文本 / 拜师 chain 4 屏 / 最终自省）。**结构性数字**（4 NPC / 4 地点 / 4 骨架 / chain 4 屏 / 1+3+1 自省）沿用；**资源调度白名单**（spirit/gold + 3 鉴定属性）用户决议；衍生议题保留：事件背景渲染层架构（委托 [[intro_全盘重新设计_预启动]]）
