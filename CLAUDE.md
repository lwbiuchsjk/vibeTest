...
- [前端表现卡牌化_MVP草案](Design/进度/前端表现卡牌化_MVP草案.md) — **草案 v3 + 工程启动 checklist**（2026-05-15）：前端交互改卡牌(引擎层保留 + 选项层新增调度);**新仓库 `zhengdao-cards`** + Design 共用决议;核心机制(事件-选项-结果三层卡循环 / 选项调度 / 鉴定+心性资源闭环 / 资源卡+标记 / 结果卡复用 outcome)就位 + **§2.2 内核明确化(普通事件玩法 / chain 叙事承担分离 — Phase B 歧途反向印证)** + MVP 4 步迭代路径;9 条 Design 分区边界 case 全部裁决;§7.8 工程启动 4 阶段 checklist(A Design 整理 / B 仓库初始化 / C 引擎迁移 / D MVP 1 期前置)待执行。**vibe-test 进入「养分源」状态**
- [当前版本完整主路径_落地进度](Design/进度/当前版本完整主路径_落地进度.md) — **Phase B 全收口**（2026-05-12，何/秦/周/程 35 事件 + sys_reflection p1 5 行 condition 差分全部真实 + 4 地点 transition + 拜师 chain 4 屏真实内容 + 最终自省 3 屏 + chain_4 outcome 武学心法承担位 + 陆师傅美术入库 + 跑测通过；程/尾声批次经验沉淀：4 outcome 核心台词一字不差话术学最深暴露 / 温水机制真正运作机制 / 精神镜面层次精细演化 / chain 跨屏伏笔与 4 包累积位对照 / 陆永恒揣测者一字识别 / 最终自省非确证收口 / §23/§24 认知陷阱沉淀）。Phase A Step 1-11 已完成（demo_mode 5 开关全生效）。**结构性数字**（4 NPC / 4 地点 / 4 骨架 / chain 4 屏 / 1+3+1 自省）全兑现；**资源调度白名单**（spirit/gold + 3 鉴定属性）用户决议；衍生议题保留：事件背景渲染层架构（委托 [[intro_全盘重新设计_预启动]]）。frontmatter 已改 `已落地`；下一阶段启动评估见各预启动进度
- [叙事重写_Kimi接入与审校方案_讨论进度](Design/进度/叙事重写_Kimi接入与审校方案_讨论进度.md) — Z 方案 3 事件测试完成，待封装 kimi_rewrite.py。**与 [[阶段3事件设计_预启动]] 启动绑定同期推进**（阶段 3 量产试点是 kimi_rewrite.py 封装的目标用户群）
- [阶段3事件设计_预启动](Design/进度/阶段3事件设计_预启动.md) — **预启动**：6 线精调完成后的事件卡颗粒度撰写，**Phase B 全收口后触发条件已满足，当前可启动**（建议作为接下来的核心工作主线之一）
- [intro_全盘重新设计_预启动](Design/进度/intro_全盘重新设计_预启动.md) — **议题 §🔵 hybrid 落地**（2026-05-12）：美术表现轨 4 NPC + 4 地点 10 张美术全部入库 + hybrid mode dispatch 接线；事件设计轨议题"事件背景渲染层架构"hybrid 方案落地（pond_* mosaic / 其他 plain PNG，FPS 60 ✓）；intro 事件编写无前置阻塞。衍生议题（mosaic 全场景化）转入 [[文字马赛克全局化_预启动]] 高优先级技术债务追踪
- [阶段4与结局统一设计_预启动](Design/进度/阶段4与结局统一设计_预启动.md) — **预启动**：阶段 4 + 双结局合并设计，待阶段 3 推进到 60-80% 触发
- [正式UI迁移_预启动](Design/进度/正式UI迁移_预启动.md) — **预启动**：正式美术风格 UI 替换当前 demo 期临时 UI；触发条件 = 开篇场景定型 + 主玩法循环验证（Phase B 已验证 1/2，等 intro 完成）
...
- [自我积累型_阶段2_拆解进度](Design/进度/自我积累型_阶段2_拆解进度.md) — 2026-05-12 归档；Step 1-3 完成 + Step 4 功能走查被 Phase B 端到端冒烟替代；38 事件作为复用候选库保留在 `test/config/intro_flow_test/`
- [叙事声部迁移_第二人称重写进度](Design/进度/叙事声部迁移_第二人称重写进度.md) — 2026-05-12 归档；38 重写产物目标 CSV（intro_flow_test）已退役为复用候选库；规范沉淀（[[叙事文风规范]] / [[叙事重写工程规范]] / narrative_validator.py）作为独立资产稳定
- [文字马赛克美术背景_落地进度](Design/进度/文字马赛克美术背景_落地进度.md) — 2026-05-12 归档；基础设施稳定 + 决策点 5/6/8 收口；hybrid 决议后衍生工作转入 [[文字马赛克全局化_预启动]]
- [阶段2回看调整_由阶段3触发_预启动](Design/进度/阶段2回看调整_由阶段3触发_预启动.md) — 2026-05-12 归档；快照范围被主路径吸收 + 触发型机制移交 [[阶段3事件设计_预启动]] 后续承担
- [事件切换呼吸感_落地进度](Design/进度/事件切换呼吸感_落地进度.md) — 2026-05-12 归档；4 增量全落地（fade 调度器 + outro 收口流程 + auto-confirm + luchengyue 美术入库）+ 跑测通过
- [UI风格快速翻调_demo期进度](Design/进度/UI风格快速翻调_demo期进度.md) — 2026-05-10 归档；议题 A/B/E 全收口（数字定性化 + 状态栏 5 印章 + OptionCard 形态 + 选项效果精调 6 子项 + 浮动汉字反馈）；议题 C 移除（心性不显示）；议题 D 迁移到事件背景渲染层架构议题；Phase 3 留代码重构期
