...
1. **设计契约**：`Design/共享/配置流水线/配置翻译指南.md` —— 真源。关键章节由 `<!-- CSV_CONTRACT_ANCHOR: xxx -->` 标记，锚点丢失会被 pre-commit 阻断（`tools/check_csv_contract_docs.py`）。当前锚点：`resolution_target_routing` / `rule_types` / `condition_types`。
...
接到以下工艺型常规任务时，先读对应必读规范再开工。**表外任务**（个性化进度延续）见下方「# 当前进度」段，进入对应进度文档顶部「新会话启动必读」段处理。

| 任务类型 | 必读 | 推荐 |
|---|---|---|
| 改 CSV 配置 | [[配置翻译指南]]、[[配置到数据落地开发规范]] | `tools/csv_validator.py` 头部注释、[[阶段配置落地流水线]] |
| 角色线精调 | [[角色线精调规范]]、[[角色线精调_经验沉淀]] | [[涌现式叙事_认知陷阱]]、[[设计稿维护规范]] |
...
- 仅覆盖**工艺型常规任务**（流程模板化、跨进度复用）。个性化活跃进度延续不进此表，由对应进度文档顶部「新会话启动必读」段承担。
- 单类必读控制在 ≤ 3 篇；超过即拆分子任务类型。
- 新增任务类型的门槛：该任务已被执行过 ≥ 2 次（避免一次性任务进入表）。

设计依据见 [[索引层优化_MVP_第一阶段]]。
...
- [前端表现卡牌化_MVP草案](Design/zhengdao-cards/进度/前端表现卡牌化_MVP草案.md) — **草案 v3 实写入盘**（2026-05-15）：前端交互改卡牌(引擎层保留 + 选项层新增调度);**新仓库 `zhengdao-cards`** + Design 共用决议;核心机制(事件-选项-结果三层卡循环 / 选项调度 / 鉴定+心性资源闭环 / 资源卡+标记 / 结果卡复用 outcome)就位 + **§2.2 内核明确化(普通事件玩法 / chain 叙事承担分离 — Phase B 歧途反向印证)** + MVP 4 步迭代路径;9 条 Design 分区边界 case 全部裁决;**§七·7.8 工程启动**:A Design 三档分区 ✅ + B zhengdao-cards 仓库初始化 ✅(Design junction 共用 + 多根工作区 + Godot 项目骨架) → C 引擎迁移 ⏳ → D MVP 1 期前置 ⏳。**vibe-test 进入「养分源」状态**
- [当前版本完整主路径_落地进度](Design/vibe-test/进度/当前版本完整主路径_落地进度.md) — **Phase B 全收口**（2026-05-12，何/秦/周/程 35 事件 + sys_reflection p1 5 行 condition 差分全部真实 + 4 地点 transition + 拜师 chain 4 屏真实内容 + 最终自省 3 屏 + chain_4 outcome 武学心法承担位 + 陆师傅美术入库 + 跑测通过；程/尾声批次经验沉淀：4 outcome 核心台词一字不差话术学最深暴露 / 温水机制真正运作机制 / 精神镜面层次精细演化 / chain 跨屏伏笔与 4 包累积位对照 / 陆永恒揣测者一字识别 / 最终自省非确证收口 / §23/§24 认知陷阱沉淀）。Phase A Step 1-11 已完成（demo_mode 5 开关全生效）。**结构性数字**（4 NPC / 4 地点 / 4 骨架 / chain 4 屏 / 1+3+1 自省）全兑现；**资源调度白名单**（spirit/gold + 3 鉴定属性）用户决议；衍生议题保留：事件背景渲染层架构（委托 [[intro_全盘重新设计_预启动]]）。frontmatter 已改 `已落地`；下一阶段启动评估见各预启动进度
- [叙事重写_Kimi接入与审校方案_讨论进度](Design/vibe-test/进度/叙事重写_Kimi接入与审校方案_讨论进度.md) — Z 方案 3 事件测试完成，待封装 kimi_rewrite.py。**与 [[阶段3事件设计_预启动]] 启动绑定同期推进**（阶段 3 量产试点是 kimi_rewrite.py 封装的目标用户群）
- [阶段3事件设计_预启动](Design/vibe-test/进度/阶段3事件设计_预启动.md) — **预启动**：6 线精调完成后的事件卡颗粒度撰写，**Phase B 全收口后触发条件已满足，当前可启动**（建议作为接下来的核心工作主线之一）
- [intro_全盘重新设计_预启动](Design/vibe-test/进度/intro_全盘重新设计_预启动.md) — **议题 §🔵 hybrid 落地**（2026-05-12）：美术表现轨 4 NPC + 4 地点 10 张美术全部入库 + hybrid mode dispatch 接线；事件设计轨议题"事件背景渲染层架构"hybrid 方案落地（pond_* mosaic / 其他 plain PNG，FPS 60 ✓）；intro 事件编写无前置阻塞。衍生议题（mosaic 全场景化）转入 [[文字马赛克全局化_预启动]] 高优先级技术债务追踪
- [阶段4与结局统一设计_预启动](Design/vibe-test/进度/阶段4与结局统一设计_预启动.md) — **预启动**：阶段 4 + 双结局合并设计，待阶段 3 推进到 60-80% 触发
- [正式UI迁移_预启动](Design/vibe-test/进度/正式UI迁移_预启动.md) — **预启动**：正式美术风格 UI 替换当前 demo 期临时 UI；触发条件 = 开篇场景定型 + 主玩法循环验证（Phase B 已验证 1/2，等 intro 完成）
- [文字马赛克_hover鱼眼_预启动](Design/vibe-test/进度/文字马赛克_hover鱼眼_预启动.md) — **预启动**：cursor 邻域字符放大 + 邻居挤开（Apple Dock 鱼眼），单 Godot 路径方案待 MVP 设计
- [Web首屏加载性能_诊断与部署方案](Design/vibe-test/进度/Web首屏加载性能_诊断与部署方案.md) — MVP 落地：nginx 自签 HTTPS 70s 首屏；COS/nip.io 路径已验证排除；CDN 等深度优化待备案解锁
- [代码重构_预启动](Design/vibe-test/进度/代码重构_预启动.md) — **预启动**：项目内容期收口后的代码重构总入口；含 Phase A 实施期 10 条工程洞察 + 5 项宏观观察 + demo 期临时收紧清单（心性完全隐藏 / 鉴定降配 / demo_mode_config 开关）；启动时机为 Phase B + 阶段 3 + 阶段 4 全部收口后
- [文字马赛克全局化_预启动](Design/vibe-test/进度/文字马赛克全局化_预启动.md) — **预启动 / 高优先级技术债务**：mosaic 是项目美术核心视觉身份；当前 hybrid 方案（pond_* mosaic / 其他事件背景 plain PNG，2026-05-12 落地）是为内容创作主线让路的**临时妥协**；内容期收口后必启动 mosaic 渲染性能修复回到全场景 mosaic 化。索引依赖设计 [[文字马赛克_hover鱼眼_预启动]] + 症状档案 [[intro_mosaic_性能排查_2026-05-11]]。**误用警示**：hybrid 状态下不要给新事件背景挂 13 层 mosaic（会立刻 FPS 5），见进度文档 §误用警示

## 已归档

- [自我积累型_阶段1_拆解进度](Design/vibe-test/进度/自我积累型_阶段1_拆解进度.md)
- [required_flag 条件类型实现](Design/vibe-test/进度/required_flag_条件类型实现.md)
- [角色精调_新会话启动指南](Design/vibe-test/进度/角色精调_新会话启动指南.md) — 2026-05-04 归档；6 角色线精调全部完成
- [叙事包扩展与demo压缩_讨论进度](Design/vibe-test/进度/叙事包扩展与demo压缩_讨论进度.md) — 2026-05-04 归档；6 角色线收口
- [填充事件批量生产_阶段2](Design/vibe-test/进度/填充事件批量生产_阶段2.md) — 2026-05-04 归档；34 个已生产 + CSV 验证
- [intro_flow_test_叙事包改造进度](Design/vibe-test/进度/intro_flow_test_叙事包改造进度.md) — 2026-05-04 归档；A-E 完成，被 [intro_全盘重新设计_预启动] 取代
- [自我积累型_阶段2_拆解进度](Design/vibe-test/进度/自我积累型_阶段2_拆解进度.md) — 2026-05-12 归档；Step 1-3 完成 + Step 4 功能走查被 Phase B 端到端冒烟替代；38 事件作为复用候选库保留在 `test/config/intro_flow_test/`
- [叙事声部迁移_第二人称重写进度](Design/vibe-test/进度/叙事声部迁移_第二人称重写进度.md) — 2026-05-12 归档；38 重写产物目标 CSV（intro_flow_test）已退役为复用候选库；规范沉淀（[[叙事文风规范]] / [[叙事重写工程规范]] / narrative_validator.py）作为独立资产稳定
- [文字马赛克美术背景_落地进度](Design/vibe-test/进度/文字马赛克美术背景_落地进度.md) — 2026-05-12 归档；基础设施稳定 + 决策点 5/6/8 收口；hybrid 决议后衍生工作转入 [[文字马赛克全局化_预启动]]
- [阶段2回看调整_由阶段3触发_预启动](Design/vibe-test/进度/阶段2回看调整_由阶段3触发_预启动.md) — 2026-05-12 归档；快照范围被主路径吸收 + 触发型机制移交 [[阶段3事件设计_预启动]] 后续承担
- [事件切换呼吸感_落地进度](Design/vibe-test/进度/事件切换呼吸感_落地进度.md) — 2026-05-12 归档；4 增量全落地（fade 调度器 + outro 收口流程 + auto-confirm + luchengyue 美术入库）+ 跑测通过
- [UI风格快速翻调_demo期进度](Design/vibe-test/进度/UI风格快速翻调_demo期进度.md) — 2026-05-10 归档；议题 A/B/E 全收口（数字定性化 + 状态栏 5 印章 + OptionCard 形态 + 选项效果精调 6 子项 + 浮动汉字反馈）；议题 C 移除（心性不显示）；议题 D 迁移到事件背景渲染层架构议题；Phase 3 留代码重构期

## 进度维护规则

1. **详情在进度文档，索引在此处**：CLAUDE.md 中每条进度只保留一行链接 + 状态摘要（≤20 字）。具体阻塞项、关键文档列表、注意事项等写在 `design_dir` 的 `vibe-test/进度/`（vibe-test 进度）或 `zhengdao-cards/进度/`（新项目进度）下的对应文档中。
2. **更新时机**：会话中产生实质性进展（完成步骤、新增/解除阻塞、新建任务）时，更新对应进度文档并同步此处的状态摘要。纯讨论不触发更新。
3. **新建进度文档**：当出现新的可独立追踪的工作项（新阶段拆解、新功能开发、新批量生产任务）时，在 `design_dir` 的 `vibe-test/进度/`（vibe-test 进度）或 `zhengdao-cards/进度/`（新项目进度）下新建文档，并在此处「活跃」区添加索引行。
...
6. **会话启动**：若延续某活跃进度，先读该进度文档顶部的「新会话启动必读」段，再开工。该段格式见 [[索引层优化_MVP_第一阶段]] §二·改造 ③。
