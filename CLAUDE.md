...
- [当前版本完整主路径_落地进度](Design/进度/当前版本完整主路径_落地进度.md) — **Phase A 已收口，Phase B 待新会话启动**：Step 1-11 全部完成（2026-05-10）。Step 11 异常分支收口：补丁 A（router 防抖）+ B（reflection 末屏 BUG 警示）+ C（tasks duration 8→20），4 项 UI 跑测全过。demo_mode 5 开关全生效。**Phase B 在新会话启动**——按进度文档「Phase B 启动议题段」5 项议题顺序讨论（议题 1 已有推荐参考 c+a 混合）。**事件数量约束边界决议**：结构性数字（4 NPC / 4 地点 / 4 骨架 / chain 4 屏 / 1+3+1 自省）沿用，测试数据集数字（每地点填充数 / 包容量）Phase B 可调。衍生议题保留：事件背景渲染层架构（委托 [[intro_全盘重新设计_预启动]]）/ 鉴定可视化（议题 E 接收）/ demo 结束界面特殊调整 / 需求 2 presentation 差分（Phase B 同期）
