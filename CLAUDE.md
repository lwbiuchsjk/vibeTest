...
- [当前版本完整主路径_落地进度](Design/进度/当前版本完整主路径_落地进度.md) — **进行中**：Step 1-3 引擎扩展 + Phase A 占位骨架数据 + 端到端冒烟通过 + **Step 10 鉴定+心性体检完成（2026-05-09）**：鉴定降配为"概念传达"层级 + 心性 demo 期完全隐藏，产出 [[鉴定 demo 期表现规约]] + [[代码重构_预启动]] + demo_mode_config 4 开关落地；衍生议题：事件背景渲染层架构（委托 [[intro_全盘重新设计_预启动]]）/ 鉴定可视化（议题 E 接收）/ demo 结束界面特殊调整 / 需求 2 presentation 差分（Phase B 同期）；剩余主线：Step 11 异常分支冒烟 → Phase B 真实文本撰写
